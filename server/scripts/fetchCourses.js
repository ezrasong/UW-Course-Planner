...
// Helper to compute current term code (e.g., "1259" for Fall 2025)
...
(async () => {
  ...
  try {
    ...
    // The API may wrap data in a .data field
    const courses = Array.isArray(res.data)
      ? res.data
      : (res.data.data || []);

    // Tag courses as programRelevant and isRequired
    // (you can keep your existing tagging logic here)
    // For brevity, we'll just write raw courses:
    const outPath = path.resolve(__dirname, '..', 'data', 'courses.json');
    fs.writeFileSync(outPath, JSON.stringify(courses, null, 2), 'utf-8');
    console.log(`Fetched ${courses.length} courses and wrote to data/courses.json`);
  }
  catch (err) {
    console.error('Error fetching course data:', err.response?.status, err.response?.statusText);
